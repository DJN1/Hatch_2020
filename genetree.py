...
class Person:
    ...
    def __str__(self):
        out = ''
        out+= "Mother: " + self.mother if self.mother != None else 'N/A' + "\n"
        out+= "Father: " + self.father if self.father != None else 'N/A' + "\n"
        out+= "Children: " + (str(self.children) if self.children != None else 'N/A') + "\n"
        out+= "Info: \n" + str(self.info)
        return out
...
def mapRelationsLevel2(lines, done):
    ...
    if "Mate" not in done and "Mate" in lines:
            ...
            lines.update( {"Mate": Person(("Mate Mother" if lines.get("Mate Mother") != None else None), ("Mate Father" if lines.get("Mate Father") != None else None), getChildren("Mate", lines, 2, []), lines.get("Mate"))})
    ...
    if "Sibling Mate" not in done and "Sibling Mate" in lines:
            ...
            lines.update( {"Sibling Mate": Person(("Sibling Mate Mother" if lines.get("Sibling Mate Mother") != None else None), ("Sibling Mate Father" if lines.get("Sibling Mate Father") != None else None), getChildren("Sibling Mate", lines, 2, []), lines.get("Sibling Mate"))})
...
grandparent.render(view=True)    #dot.node(p.info.relation, shape=('box' if p.info.sex == 'M' else 'circle'), color=('blue' if p.info.sex == 'M' else 'pink'))
